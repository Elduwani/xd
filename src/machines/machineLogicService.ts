...
export async function checkReportsTable(props: { ssn: string, bureauName: string }) {
   console.log("Checking for an existing report....");
   try {
      // const report = await collections.creditReports?.findOne({
      //    ssn: props.ssn,
      //    bureauName: props.bureauName,
      // });
      const report = {}
...
export async function checkBureauService(props: { ssn: string; bureauName: string }) {
   switch (props.bureauName) {
      case "GavUnion":
